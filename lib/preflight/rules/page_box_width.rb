...
module Preflight
  module Rules
    ...
    #
    #     rule Preflight::Rules::PageBoxWidth, :MediaBox, 100, :mm
    #     rule Preflight::Rules::PageBoxWidth, :TrimBox, 600, :pts
    #     rule Preflight::Rules::PageBoxWidth, :CropBox, 5, :in
    #     rule Preflight::Rules::PageBoxWidth, :MediaBox, 100..101, :mm
    #     rule Preflight::Rules::PageBoxWidth, :TrimBox, 600..700, :pts
    #     rule Preflight::Rules::PageBoxWidth, :CropBox, 5..6, :in
    #   end
    #
    class PageBoxWidth
      ...
      def page=(page)
        ...
        if dict[@box]
          ...
          if !@width.include?(box_width)
            @issues << Issue.new(1, "sizeview", "O arquivo deve ter a largura de #{@orig_width}#{@units}", self, :page  => page.number,
                                                                                        :box   => @box,
                                                                                        :width => @orig_width,
                                                                                        :units => @units)
          end
      ...
      def mm_to_points(width)
        case width
        when Numeric then Range.new(mm2pt(width)-0.1, mm2pt(width)+0.1)
